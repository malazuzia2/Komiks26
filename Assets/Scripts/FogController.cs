...
public class FogController : MonoBehaviour
{
    ...
    //}
    private Lantern cachedLantern; // Dodaj to pole w klasie

    void Update()
    {
        // Optymalizacja: szukamy latarni tylko raz, jeœli jeszcze jej nie mamy
        if (cachedLantern == null)
        {
            cachedLantern = FindFirstObjectByType<Lantern>();
        }

        bool isLanternOn = (cachedLantern != null && cachedLantern.flashlightLight.enabled);
        bool activeBuoyNearby = false;
        ...
        // Aktualizacja Particle System
        if (fogParticles != null)
        {
            float t = (currentRadius - minRadius) / (maxRadius - minRadius);

            // Pobieramy modu³y bezpoœrednio z instancji (to naprawia b³¹d)
            ...
            emission.rateOverTime = Mathf.Lerp(400, 50, t);

            var shape = fogParticles.shape;
            shape.radius = currentRadius;
        }

        // Ustawienia mg³y w renderowaniu
        RenderSettings.fogStartDistance = currentRadius * 0.1f;
        RenderSettings.fogEndDistance = currentRadius * 1.1f;
    }
